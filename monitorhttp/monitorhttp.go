...
type HTTPConnection struct {
    // DomainNameLookupTime float64 //:= t1.Sub(t0)
    // DomainNameLookupTime time.Duration //:= t1.Sub(t0)
    DomainNameLookupTime float64 //:= t1.Sub(t0)
    TCPConnectionTime float64 //:= t2.Sub(t1)
    ConnectTime float64 //:= t2.Sub(t0)
    PreTransferTime float64 //:= t3.Sub(t0)
    // StartTransferTime int64 //:= t4.Sub(t0)
    StartTransferTime float64 //:= t4.Sub(t0)
    ServerProcessingTime float64//:= t4.Sub(t3)
    TLSHandshakeTime float64//:= t6.Sub(t5)
    ContextTransferTime  float64//:= t7.Sub(t4)
    TotalTime float64//:= t7.Sub(t0)
    StatusCode int16
    TextRegexMatch bool
    IPAddress net.IP
...
func visit(url *url.URL) {
	...
	fmt.Println()
	fmt.Printf("Raw times: %v, %v, %v, %v, %v, %v, %v", t0, t1, t2, t3, t4, t5, t6)
	fmt.Println("\n\n\n")
	td1 := t1.Sub(t0)
	td2 := t2.Sub(t0)
	td3 := t3.Sub(t0)
	...
	context_transfer := t7.Sub(t4)
	total := t7.Sub(t0)
	//DNS LOOKUP TCP Connection   TLS Handshake   Server Processing   Content Transfer
	// fmt.Printf("Subtracted times: %v, %v, %v, %v, %v, %v, %v", t0, td1, td2, td3, td4, td5, td6)
	fmt.Printf("Subtracted times: %v, %v, %v, %v, %v, %v", td1, td2, td3, td4, td5, td6, td7)
	fmt.Println("\n\n")
	fmt.Println(namelookup, tcp_connection, connect, pretransfer, starttransfer, server_processing, tls_handshake, context_transfer, total)
	fmt.Println("\n\n")
	fmt.Println(namelookup, connect, pretransfer, starttransfer, total)
	fmt.Println("\n\n")
	var r = HTTPConnection{}

    // r.DomainNameLookupTime := t1.Sub(t0)
    // r.SetDomainNameLookupTime(t1.Sub(t0))
	fmt.Println(reflect.TypeOf(r))
	fmt.Println("%v\n\n", tcp_connection)
    // r.DomainNameLookupTime := t1.Sub(t0)
    // r.DomainNameLookupTime = 1
    r.DomainNameLookupTime = (t1.Sub(t0)).Seconds()
    r.TCPConnectionTime = (t2.Sub(t1)).Seconds()
    r.ConnectTime = (t2.Sub(t0)).Seconds()
    r.PreTransferTime = (t3.Sub(t0)).Seconds()
    // r.StartTransferTime = t4.Sub(t0)
    // r.StartTransferTime = total.Microseconds()
    r.StartTransferTime = (t4.Sub(t0)).Seconds()
    r.ServerProcessingTime = (t4.Sub(t3)).Seconds()
    r.TLSHandshakeTime = (t6.Sub(t5)).Seconds()
    r.ContextTransferTime = (t7.Sub(t4)).Seconds()
    r.TotalTime = (t7.Sub(t0)).Seconds()
    r.StatusCode = 200
    r.TextRegexMatch = true
	ip := "10.1.1.1"
    r.IPAddress = net.ParseIP(ip)
	fmt.Printf("r: %v", r)
	fmt.Printf("rTotalTime: %v", r.TotalTime)
// busk
